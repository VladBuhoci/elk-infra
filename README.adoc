...
[IMPORTANT]
====
If the _"http-apache"_ pipeline (mentioned in later examples) is tested for the first time, a custom user needs to be created because it is required by Logstash for storing data in an Elasticsearch index.
...
. Example #3
+
The _"http-apache"_ pipeline also has support for multiline error logs, Java-style (sample data can be found in the `./test-data/` folder):
+
[source,bash]
----
curl \
...
[NOTE]
====
If you want to visualize the data processed by the _"http-apache"_ pipeline in Kibana, you can do so by going to the _Analytics / Discover_ page of Kibana and selecting the appropriate data view.
